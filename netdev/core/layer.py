...
class Layer:
    ...
    async def enter(self) -> str:
        ...
        self._logger.info(
            "Host %s: %s cli mode: Enter to this cli mode", self.host, self.cli_mode.name,
        )
    ...
    async def exit(self) -> str:
        ...
        self._logger.info(
            "Host %s: %s cli mode: Exit from this cli mode", self.host, self.cli_mode.name,
        )
    ...
    async def commit(self) -> str:
        """ Commit changes for this cli mode if layer supports this"""

        output = ""
        if self._commit_func:
            self._logger.info(
                "Host %s: %s cli mode: Commit changes", self.host, self.cli_mode.name)
...
class LayerManager:
    ...
    def __init__(
        self, device_stream: DeviceStream, cli_modes: IntEnum, check_func: Callable[[str], str],
    ):
        self._device_stream = device_stream
        self.cli_modes = cli_modes
        ...
        if current_cli_mode < target_cli_mode:
            while current_cli_mode != target_cli_mode:
                ...
                if current_cli_mode != real_cli_mode:
                    raise SwitchError(
                        self.host, f"Cannot enter to {current_cli_mode}")
            ...
            while current_cli_mode != target_cli_mode:
                ...
                current_cli_mode = layer.cli_mode
                real_cli_mode = await self._check_cli_mode()
                if current_cli_mode != real_cli_mode:
                    raise SwitchError(
                        self.host, f"Cannot exit from {current_cli_mode}")

        self._current_cli_mode = current_cli_mode
        return output

    async def _check_cli_mode(self) -> IntEnum:
        """ Check current cli mode """
        self._logger.info(
            "Host %s: Recognizing the current cli mode", self.host)
        buf = await self._device_stream.send_commands("\n", strip_prompt=False)
        current_cli_mode = await self._check_func(buf)
        self._logger.info("Host %s: Recognized cli mode is %s",
                          self.host, current_cli_mode.name)
        return current_cli_mode

    async def curreng_cli_mode(self) -> IntEnum:
        """ Get current cli mode """
        self._current_cli_mode = self._current_cli_mode or await self._check_cli_mode()
        self._logger.info("Host %s: Current cli mode is %s",
                          self.host, self._current_cli_mode.name)
